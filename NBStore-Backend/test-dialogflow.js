...
async function testDialogflowAPI() {
	try {
        ...
        for (let i = 0; i < testCases.length; i++) {
            ...
            try {
                const messageResponse = await axios.post(`${API_URL}/dialogflow/message`, {
                    message: testMessage,
                    sessionId: sessionId
                });
                ...
                console.log(`🎯 Intent: ${data.intent} (${Math.round(data.confidence * 100)}%)`);
                console.log('---');
                ...
                console.log(`❌ Error with "${testMessage}":`, error.response?.data?.message || error.message);
                console.log('---');
            }
        }
        ...
        console.log('\n🎉 Testing completed!');
        console.log('\n💡 Tips:');
        console.log('- Fallback responses được sử dụng khi Dialogflow chưa cấu hình');
        console.log('- Cấu hình Dialogflow để có responses tốt hơn');
        console.log('- Kiểm tra DIALOGFLOW_INTENTS_SETUP.md để biết cách setup');
